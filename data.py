...
def format_date(date_str):
    try:
        # Example format: "09/20/2023 12:00:00 AM" or "20230920"
        return datetime.strptime(date_str, "%m/%d/%Y %I:%M:%S %p").isoformat()
    except ValueError:
        try:
...
def convert_to_fixture(data):
    ...
    for index, item in enumerate(data, start=1):
        fixture = {
            "model": "pages.FoodFacilityPermit",
            "pk": index,
            "fields": {
                ...
                "dayshours": item.get("dayshours", ""),
                "noi_sent": item.get("NOISent", ""),
                "approved": format_date(item.get("Approved", "")),
                "received": item.get("Received", ""),
                "prior_permit": item.get("PriorPermit", "0") == "1",
                "expiration_date": format_date(item.get("ExpirationDate", "")),
                "location": item.get("Location", ""),
                "fire_prevention_districts": item.get("Fire Prevention Districts", None),
                "police_districts": item.get("Police Districts", None),
                "supervisor_districts": item.get("Supervisor Districts", None),
                "zip_codes": item.get("Zip Codes", None),
                "neighborhoods_old": item.get("Neighborhoods (old)", ""),
            }
        }
